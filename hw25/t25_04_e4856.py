...
def create_graph(matrix: list, n: int) -> list:
...
def get_weight_and_path(graph: list, start: int, end: int) -> list:
    n = len(graph)
...
if __name__=="__main__":
    with open('input.txt', 'r') as file:
        ...
        s, f = list(map(int, file.readline().split()))
        
        matrix = [[0]*n for _ in range(n)]
        for _ in range(m):
            b, e, w = list(map(int, file.readline().split()))
            matrix[b-1][e-1] = w
            matrix[e-1][b-1] = w

        graph = create_graph(matrix, n)
        weight_and_path = get_weight_and_path(graph, s-1, f-1)
        
        for line in weight_and_path: print(*line)
